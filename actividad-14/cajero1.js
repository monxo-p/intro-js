...
function atmMachinePassword() {
  var pswEntry = parseInt(
    prompt("Dear " + username + ", enter your 4 digit PIN")
  );
  ...
  if (correct_pass.test(userInput)) {
    ...
    while (!correct_pass.test(userInput)) {
      if (passTry === 1) {
        alert("Incorrect PIN.");
        alert(
          "Maximum tries exceeded! Your account has been locked. Contact your bank for support."
        );
        exit();
        break;
      } else {
